...
test_that("day 2 example programs", {
  runIntCodeComputer(c(1,9,10,3,2,3,11,0,99,30,40,50),
                     done = test_arg_is(c(3500,9,10,70,
                                                  2,3,11,0,
                                                  99,
                                                  30,40,50)))

  runIntCodeComputer(c(1,0,0,0,99),
                     done = test_arg_is(c(2,0,0,0,99)))

  runIntCodeComputer(c(2,3,0,3,99),
                     done = test_arg_is(c(2,3,0,6,99)))

  runIntCodeComputer(c(2,4,4,5,99,0),
                     done = test_arg_is(c(2,4,4,5,99,9801)))

  runIntCodeComputer(c(1,1,1,4,99,5,6,0,99),
...
test_that("comparators", {
  ...
  runIntCodeComputer(prog1, iccin = iccInput(7), iccout = test_arg_is(0))
  runIntCodeComputer(prog1, iccin = iccInput(8), iccout = test_arg_is(1))

  # Using position mode, consider whether the input is less than 8;
  # output 1 (if it is) or 0 (if it is not).
  prog2 <- c(3,9,7,9,10,9,4,9,99,-1,8)

  runIntCodeComputer(prog2, iccin = iccInput(7), iccout = test_arg_is(1))
  runIntCodeComputer(prog2, iccin = iccInput(8), iccout = test_arg_is(0))
  runIntCodeComputer(prog2, iccin = iccInput(9), iccout = test_arg_is(0))

  # Using immediate mode, consider whether the input is equal to 8;
  # output 1 (if it is) or 0 (if it is not).
  prog3 <- c(3,3,1108,-1,8,3,4,3,99)

  runIntCodeComputer(prog3, iccin = iccInput(7), iccout = test_arg_is(0))
  runIntCodeComputer(prog3, iccin = iccInput(8), iccout = test_arg_is(1))

  # Using position mode, consider whether the input is less than 8;
  # output 1 (if it is) or 0 (if it is not).
  prog4 <- c(3,9,7,9,10,9,4,9,99,-1,8)

  runIntCodeComputer(prog4, iccin = iccInput(7), iccout = test_arg_is(1))
  runIntCodeComputer(prog4, iccin = iccInput(8), iccout = test_arg_is(0))
  runIntCodeComputer(prog4, iccin = iccInput(9), iccout = test_arg_is(0))
})
...
test_that("jumps", {
  ...
  runIntCodeComputer(prog1, iccin = iccInput(0), iccout = test_arg_is(0))
  runIntCodeComputer(prog1, iccin = iccInput(1337), iccout = test_arg_is(1))

  prog2 <- c(3,3,1105,-1,9,1101,0,0,12,4,12,99,1) # (using immediate mode)

  runIntCodeComputer(prog2, iccin = iccInput(0), iccout = test_arg_is(0))
  runIntCodeComputer(prog2, iccin = iccInput(1337), iccout = test_arg_is(1))
})
...
test_that("day 6", {
  ...
  runIntCodeComputer(prog, iccin = iccInput(7), iccout = test_arg_is(999))
  runIntCodeComputer(prog, iccin = iccInput(8), iccout = test_arg_is(1000))
  runIntCodeComputer(prog, iccin = iccInput(9), iccout = test_arg_is(1001))
})
